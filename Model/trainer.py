...
pred_y_test = torch.max(outputs1, dim=1)[1].data.cpu().numpy()
# target_y_test = labels_v1.data.cpu().numpy()
# pred_y_test1 = torch.softmax(outputs_v1, dim=1)[:, 1].data.cpu().numpy()

result = torch.from_numpy(pred_y_test).type(torch.FloatTensor).reshape(y_test.shape[0],y_test.shape[1],y_test.shape[2]).cuda()
...
#     ))
# torch.save(model.state_dict(), 'm_2.params')
