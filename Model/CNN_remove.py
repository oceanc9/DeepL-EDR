...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=2):
        ...
        self.in_planes = 53
        self.layer3 = self._make_layer(block[0], 64, num_blocks[0], stride=1)
        self.layer4 = self._make_layer(block[1], 32, num_blocks[1], stride=1)
    ...
    def forward(self, x, unrela_x):
        x = x.permute(0, 3, 2, 1)
        out = self.layer4(self.layer3(x)).permute(0, 3, 2, 1).reshape(-1, 32)
        out = self.fc1(out)
        return out

def ResNet50():
    return ResNet([BasicBlock1,BasicBlock2], [1, 3])
...
setup_seed(0)
model = ResNet50().cuda()
model_dict = torch.load('m_1.params')
...
#         ))
#     if epoch == 195:
#         torch.save(model.state_dict(), 'm_1.params')
