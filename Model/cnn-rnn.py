...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=2):
        ...
        self.in_planes = 53
        self.layer5 = self._make_layer(block[0], 64, num_blocks[2], stride=1)
        self.layer6 = self._make_layer(block[1], 32, num_blocks[1], stride=1)
        self.in_planes = 53
        self.layer7 = self._make_layer(block[0], 64, num_blocks[2], stride=1)
        ...
        #
        # self.feature0 = nn.Sequential(  # input shape (90, 16, 2)
        #     nn.Conv2d(
        #         in_channels=32,  # input height
        #         out_channels=106,  # n_filters
        #         kernel_size=3,  # filter size
        #         stride=1,  # filter movement/step
        #         padding=1,
        #     ),  # output shape (32, 16, 2)
        #     nn.BatchNorm2d(106),
        #     nn.ReLU(inplace=True),
        #     nn.MaxPool2d(kernel_size=(8, 1)),  # output shape (64, 16, 2)
        # )
        # self.feature4 = nn.Sequential(  # input shape (90, 16, 2)
        #     nn.Conv2d(
        #         in_channels=32,  # input height
        #         out_channels=106,  # n_filters
        #         kernel_size=3,  # filter size
        ...
        # self.embed0 = nn.Sequential(torch.nn.Linear(20, 10),nn.ReLU(inplace=True))
        # self.embed4 = nn.Sequential(torch.nn.Linear(20, 10),nn.ReLU(inplace=True))
        # self.fc0 = nn.Sequential(nn.Linear(434, 200),nn.ReLU(inplace=True))
        # self.fc4 = nn.Sequential(nn.Linear(434, 200),nn.ReLU(inplace=True))
        # self.fc00 = nn.Sequential(nn.Linear(200, 2),nn.Sigmoid())
        # self.fc44 = nn.Sequential(nn.Linear(200, 2),nn.Sigmoid())
    ...
    def forward(self, x, unrela_x):
        ...
        output = torch.stack((out2, out3, out4))
        ...
        # output0 = self.feature0(output0).reshape(output0.shape[0], 2 * 2 * 106)
        # embedding = self.embed0(unrela_x)
        ...
        # output4 =self.feature4(output4).reshape(output4.shape[0], 2 * 2 * 106)
        # embedding = self.embed4(unrela_x)
        # output4 = self.fc44(self.fc4(torch.cat((output4, embedding), 1)))
        return output1, output2, output3

def ResNet50():
    return ResNet([BasicBlock1,BasicBlock2,BasicBlock0], [1, 1, 3])
...
for batch_size in BATCH:
    ...
    for epoch in range(epochs):
        ...
        x_train, unrela_x_train, y_train1, y_train2, y_train3 = x_train[idx_shuffle],unrela_x_train[idx_shuffle], \
                                                                                     y_train1[idx_shuffle],\
                                                                       y_train2[idx_shuffle], y_train3[idx_shuffle]
        ...
        for i in range(train_size // batch_size + 1):
            ...
            optimizer.zero_grad()
            outputs1, outputs2, outputs3 = model(inputs,aux)
            ...
            loss = para[0]* loss_func0(outputs_v1, labels_v1) + para[1]*loss_func0(outputs_v2, labels_v2) + para[2] * loss_func0(outputs_v3 ,labels_v3)
        ...
        with torch.no_grad():
            # Train
            outputs1, outputs2, outputs3 = model(x_train, unrela_x_train)
            ...
            # print(loss_func0(outputs_v1, labels_v1),\
            # loss_func0(outputs_v2, labels_v2) , loss_func0(outputs_v3, labels_v3))
            # Test
            outputs1, outputs2, outputs3 = model(x_test,unrela_x_test)
        ...
        if metrics.f1_score(target_y_test1, pred_y_test1)>0.755 and metrics.f1_score(target_y_test2, pred_y_test2)>0.855 and metrics.f1_score(target_y_test3, pred_y_test3)>0.795:
            print(epoch)
            print('拆除旧修复体')
            print("Train/Test  Acc {:.4f} / {:.4f}, Recall {:.4f} / {:.4f}, Precision {:.4f} / {:.4f}, F1 {:.4f} / {:.4f}, AUC {:.4f} / {:.4f}".format(
