...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=2):
        ...
        self.fc2 = nn.Linear(16 * 2, 2)
        self.fc3 = nn.Linear(16 * 2, 2)
    ...
    def forward(self, x, unrela_x):
        ...
        h0 = torch.zeros(2, output.size(0), 16).cuda()  # 同样考虑向前层和向后层
        c0 = torch.zeros(2, output.size(0), 16).cuda()
        output, _ = self.lstm(output, (h0, c0))  # LSTM输出大小为 (batch_size, seq_length, hidden_size*2)
        output = output.permute(1, 0, 2)
        # 牙周治疗
        ...
        # 拆除旧修复体
        output1 = self.fc1(output[1])
        # 拔除
        output2 = self.fc2(output[2])
        # 牙体牙髓
        output3 = self.fc3(output[3])
        # 修复科复诊
...
beta = 1e-4
# scheduler = lr_scheduler.CosineAnnealingLR(optimizer,T_max=40,eta_min=0,last_epoch=-1)
scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[30, 70], gamma=0.1, last_epoch=-1)
for epoch in range(epochs):
    print(epoch)
    random.shuffle(idx_shuffle)
    x_train, unrela_x_train, y_train0, y_train1, y_train2, y_train3, y_train4 = x_train[idx_shuffle],unrela_x_train[idx_shuffle], \
                                                                                y_train0[idx_shuffle], y_train1[idx_shuffle],\
                                                                   y_train2[idx_shuffle], y_train3[idx_shuffle], y_train4[idx_shuffle]
    ...
    for i in range(train_size // batch_size + 1):
        ...
        loss = loss_func0(outputs0, labels0) + loss_func0(outputs_v1, labels_v1)+\
